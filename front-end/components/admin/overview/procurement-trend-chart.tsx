...
export default function ProcurementVolumeChart() {
	return (
		<div data-slot="procurement-volume-chart" className="rounded-xl border border-border bg-surface p-6">
			...
			<div className="mt-4 flex items-center gap-5">
				<span className="flex items-center gap-1.5 font-mono text-xs text-foreground-subtle">
					...
					Total de Licitações (R$ Milhões)
				</span>
				<span className="flex items-center gap-1.5 font-mono text-xs text-foreground-subtle">
					<span className="size-2.5 rounded-full bg-primary" />
					Licitações ME/EPP (R$ Milhões)
				</span>
			</div>
			...
			<div className="mt-4 h-80 w-full">
				<ResponsiveContainer width="100%" height="100%">
					<LineChart data={data}>
						...
						/>
						<Tooltip
							formatter={(tickValue: unknown) => `R$ ${Number(tickValue ?? 0)}M`}
							contentStyle={{
								borderRadius: 8,
